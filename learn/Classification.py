...
class Classification():
    ...
    def _getClassifier(self):
        ...
        classifierArgs = self.classifierArgs
    ...
    def _saveResults(self, resultPath, details=True):
        ...
        if details:
            ...
            if self.hiddenDetails != None:
                self._saveDetails(self.meta, self.hiddenDetails.get("predictions", None), self.hiddenDetails.get("importances", None), "hidden", featureByIndex)
            fold = 0
            for extra in self.extras:
                self._saveDetails(self.meta, extra.get("predictions", None), extra.get("importances", None), fold, featureByIndex)
                fold += 1
        else:
